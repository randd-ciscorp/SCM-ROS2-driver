...
{
    std::vector<float> tofData = std::vector<float>(tofIntDev.getInfo().width * tofIntDev.getInfo().height);
    tofIntDev.getData(reinterpret_cast<uint8_t*>(tofData.data()));

    std::vector<uint8_t> rgbData = std::vector<uint8_t>(rgbIntDev.getInfo().width * rgbIntDev.getInfo().height);
    ...
    if (isAligned)
    {
        ...
        cis::rgbd_fusion(reinterpret_cast<char*>(tofData.data()), reinterpret_cast<char*>(rgbData.data()), reinterpret_cast<char*>(outData), &tofParams, &tofCalib, &fusionData);
    }
    else
    {
